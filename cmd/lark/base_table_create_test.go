...
func TestBaseTableCreateCommand(t *testing.T) {
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			t.Fatalf("expected POST, got %s", r.Method)
		}
		if r.URL.Path != "/open-apis/bitable/v1/apps/app_1/tables" {
		...
		if !ok {
			t.Fatalf("missing table payload: %#v", payload["table"])
		}
		if table["name"] != "MyTable" {
			t.Fatalf("unexpected table name: %#v", table["name"])
		}
	...
	if !strings.Contains(buf.String(), "tbl_1") {
