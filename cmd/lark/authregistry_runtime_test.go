...
func TestUserOAuthScopesForCommand(t *testing.T) {
	...
	if want := []string{"offline_access", "docs:document.comment:read", "drive:drive.metadata:readonly", "drive:drive.search:readonly", "space:document:retrieve"}; !reflect.DeepEqual(scopes, want) {
		t.Fatalf("scopes=%v, want %v", scopes, want)
	}
