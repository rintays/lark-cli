...
func newMeetingCmd(state *appState) *cobra.Command {
	...
	cmd.AddCommand(newMeetingGetSDKCmd(state))
	return cmd
}
...
func newMeetingGetSDKCmd(state *appState) *cobra.Command {
	var meetingID string
	var withParticipants bool
	var withMeetingAbility bool
	var userIDType string
	...
	cmd := &cobra.Command{
		Use:   "get-sdk <meeting-id>",
		Short: "Get meeting details via the SDK",
		Args:  cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) > 0 {
				if meetingID != "" && meetingID != args[0] {
